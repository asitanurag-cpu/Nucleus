...
import type { EnergySignalType, IntelSignal, WatchlistEntry, TTOInstitution } from "@/lib/types/signals";
...
interface Props {
  ...
  ttoInstitutions: TTOInstitution[];
}

export default function EnergySignalsClient({ signals, watchlist, ttoInstitutions }: Props) {
  ...
  const energySignals = useMemo(
    ...
    [signals, selectedTypes, minScore, preCompanyOnly, institutionFilter]
  );
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-10 lg:px-6">
      ...
      {/* Watchlist */}
      <WeeklyWatchlistPanel vertical="energy" entries={watchlist} allSignals={signals} />
