...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  if (!playbook) return { title: "Playbook Not Found | Nucleus" };
  return {
...
export default async function PlaybookPage({ params }: { params: Params }) {
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-8 lg:px-6 lg:py-12">
      <Link href="/playbooks" className="mb-8 inline-flex items-center gap-1.5 text-sm text-nucleus-text-muted transition-colors hover:text-nucleus-text-primary">
        <ArrowLeft className="h-3.5 w-3.5" /> Back to Playbooks
      </Link>
